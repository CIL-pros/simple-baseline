...
def extract_data(filename, start_im, end_im):
...
def extract_labels(filename, start_lab, end_lab):
    """Extract the labels into a 1-hot matrix [image index, label index]."""
    gt_imgs = []
    for i in range(start_lab, end_lab + 1):
        imageid = "satImage_%.3d" % i
        image_filename = filename + imageid + ".png"
        if os.path.isfile(image_filename):
            print('Loading ' + image_filename)
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    test_data_filename = 'data/test_images/'

    # Extract it into numpy arrays.
    train_data = extract_data(train_data_filename, 1, TRAINING_SIZE)
    eval_data = extract_data(train_data_filename, TRAINING_SIZE + 1, TRAINING_SIZE + VALIDATION_SIZE)
    train_labels = extract_labels(train_labels_filename, 1, TRAINING_SIZE)
    eval_labels = extract_labels(train_labels_filename, TRAINING_SIZE + 1, TRAINING_SIZE + VALIDATION_SIZE)
    ...
    with tf.Session() as s:
        ...
        if RESTORE_MODEL:
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range(int(train_size / BATCH_SIZE)):
                    ...
                    # print('Epoch {0:}: {1:3.2f}%'.format(iepoch, step / int(train_size / BATCH_SIZE) * 100))
                    ...
                    if step % RECORDING_STEP == 0:
                        feed_dict = {train_data_node: eval_data,
                                     train_labels_node: eval_labels}
                        ...
                        print('Validation loss: %.3f, learning rate: %.6f' % (l, lr))
                        print('Validation error: %.1f%%' % error_rate(predictions, eval_labels))
        ...
        directory = os.fsencode(test_data_filename)
        for file in sorted(os.listdir(directory)):
            filename = os.fsdecode(file)
            img = mpimg.imread(test_data_filename + filename)
            img_prediction = get_prediction(img)
            scipy.misc.imsave(prediction_testing_dir + 'label_' + filename, img_prediction)
            cimg = concatenate_images(img, img_prediction)
            Image.fromarray(cimg).save(prediction_testing_dir + "prediction_" + filename)
            oimg = make_img_overlay(img, img_prediction)
            oimg.save(prediction_testing_dir + "overlay_" + filename)
